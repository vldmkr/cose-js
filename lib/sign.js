...
async function doSign(SigStructure, signer, alg) {
  ...
  if (AlgFromTags[alg].sign.startsWith('ES')) {
    ...
    //console.log('👆ToBeSigned' + ToBeSigned.length + ' ' + ToBeSigned.toString('hex'));
    // const ec = new EC(COSEAlgToNodeAlg[AlgFromTags[alg].sign].sign);
    let signature;
    signature = await p256.sign(ToBeSigned, signer.key.d);

    sig = Buffer.concat([i2osp(signature.r, p256.CURVE.nByteLength), i2osp(signature.s, p256.CURVE.nByteLength)]);
...
function doVerify(SigStructure, verifier, alg, sig) {
  ...
  if (AlgFromTags[alg].sign.startsWith('ES')) {
    ...
    const pub = bytesToHex(compressBuf);
    const isValid = p256.verify(sig, msgHash, pub) === true;
    if (!isValid) {
