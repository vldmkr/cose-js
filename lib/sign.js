...
const { bytesToHex, hexToNumber } = require('@noble/curves/abstract/utils');
...
const ec_set = { p256: p256, p384: p384, p521: p521 };
const ed_set = { ed25519: ed25519, ed448: ed448 };
...
async function doSign(SigStructure, signer, alg) {
  ...
  if (AlgFromTags[alg].sign.startsWith('ES')) {
    ...
    const ec = ec_set[COSEAlgToNodeAlg[AlgFromTags[alg].sign].sign];

    let signature;
    signature = await ec.sign(ToBeSigned, signer.key.d);
    ...
    const ed = ed_set['ed25519'];
    let signature;
    signature = await ed.sign(ToBeSigned, signer.key.d);
    sig = signature;
  } else {
...
function doVerify(SigStructure, verifier, alg, sig) {
  ...
  if (AlgFromTags[alg].sign.startsWith('ES')) {
    ...
    // TODO:choosing curve by verification key
    const ed = ed_set['ed25519'];
    const isValid = ed.verify(sig, ToBeSigned, verifier.key.x) === true;
    if (!isValid) {
      throw new Error('Signature missmatch');
    } else {
      console.log(isValid);
